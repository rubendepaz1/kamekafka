...
open class KafkaUnitTestBase<In,Out>(filterSalutes: (KStream<String, In>) -> KStream<String, Out>, inClass: Class<In>, outClass: Class<Out>) {
    protected val inputTopic: TestInputTopic<String, In>
    protected val outputTopic: TestOutputTopic<String, Out>
    init {
        val builder = StreamsBuilder()
        val input = builder.stream("input-topic", Consumed.with(Serdes.String(), Serdes.String()))
        com.kanekotic.kafkaapp.transformations.filterSalutes(input).to("result-topic")
